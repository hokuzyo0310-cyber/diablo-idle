...
func _roll_rarity(area_level: int, magic_find: float) -> Dictionary:
	...
	var tiers: Array = rarity_tiers if not rarity_tiers.is_empty() else AffixPresets.get_all_rarity_tiers()

	# 计算总权重
	var total_weight: float = 0.0
	for tier: Dictionary in tiers:
		# 区域等级加成: 每级 +2%
		var weight_mod: float = 1.0 + area_level * 0.02
		# 魔法发现加成: 每点 MF +1%
		var mf_mod: float = 1.0 + magic_find / 100.0
...
func _get_available_affixes(affix_type: String, _base: Dictionary) -> Array[Dictionary]:
	var source: Array = affix_pools.get(affix_type, []) if affix_type == "prefix" else affix_pools.get(affix_type, [])
...
func _pick_base_item(_area_level: int) -> Dictionary:
	var items: Array = base_items if not base_items.is_empty() else EquipmentPresets.get_all_base_items()
	if items.is_empty():
		return {}
